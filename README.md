...
- 可用 `取消本页已隐藏` 只恢复当前搜索结果里已隐藏的 release，不清空其他搜索页的隐藏记录。
...
PowerArrPlus/
  ...
  docker-compose.yml                          204 服务器部署配置
...
- 过滤服务: `http://192.168.2.204:17896`
- Redis: `192.168.2.204:6379`

204 Docker 部署：

```bash
cd /root/powerarrplus
...
搜索后顶部 `Donate` 左侧会出现 `Seen Filter` 工具条。勾选结果后点 `隐藏选中` 只会写入隐藏记录并提示，下次手动搜索时这些结果会被隐藏，不会自动重新触发 Prowlarr 搜索请求。点 `取消本页已隐藏` 会恢复当前搜索结果里已被过滤服务隐藏的 release，下次手动搜索时显示。
...
| 变量 | 默认值 |
| --- | --- |
| `POWERARR_PLUS_BIND` | `127.0.0.1`，Docker 部署为 `0.0.0.0` |
...
油猴脚本里的服务地址默认是 `http://192.168.2.204:17896`。如需临时改地址，可以在 Prowlarr 页面控制台执行：

```javascript
localStorage.setItem("powerarrPlusServiceOrigin", "http://192.168.2.204:17896");
```

如果之前手工设置过本地地址，需要删除或改掉旧值：
...
该页面会加载同一份 `userscripts/prowlarr_seen_filter.user.js`，用 XHR 模拟 Prowlarr 搜索结果和左侧原生选择框，用来检查工具条、勾选框、严格去重、`隐藏选中` 和 `取消本页已隐藏` 流程是否会卡住页面。
